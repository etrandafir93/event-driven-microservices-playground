...
class OutboxKafkaPublisher {
    ...
    public OutboxKafkaPublisher(Outbox outbox, ProducerFactory<?, ?> producerFactory) {
        this.outbox = outbox;
        this.kafkaTemplate = stringKafkaTempalte(producerFactory);
    }
    ...
    private void publishAndUpdate(OutboxMessage msg) {
        ...
        boolean sent = kafkaTemplate.send(kafkaMsg)
            .thenApply(res -> {
